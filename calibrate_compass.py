...
if data_file[-4] == u'.':
    OUTPUT_PNG = data_file[0:-4] + u'.png'
else:
    OUTPUT_PNG = data_file + u'.png'
...
print(GNUPLOT_SCRIPT % (
    str(M1).replace("\n", "\n# "),
    json.dumps(M1.tolist()),
    warning,
    data_file,
    OUTPUT_PNG,
    MAX_SCALE, MAX_SCALE, MAX_SCALE, MAX_SCALE, MAX_SCALE,
    cx, cy, a, b, np.degrees(phi),
    cx, cy, a*2, b*2, np.degrees(phi),
    cx, cy, max(a, b),
    max(a, b),
    cx, cy, ax, ay, cx, cy, bx, by,
    cx, cy, x1, y1, cx, cy, x2, y2,
    x1, y1, x2, y2,
    x2-cx, y2-cy,
    x2-cx, y2-cy
))
